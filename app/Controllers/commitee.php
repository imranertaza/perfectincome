...
class commitee extends CI_Controller {
	...
	public function index()
	{
		...
		$data['base_url'] = base_url().'commitee/index/';
		$data['total_rows'] = $this->db->query("SELECT * FROM `commitee` WHERE `type` = 'm_member' ORDER BY `tec_id` ASC")->num_rows();
		$data['per_page'] = 10; 
		$data['uri_segment'] = 3;
		$data['num_links'] = 5;
		$get_segment_uri = $this->uri->segment(3);
		$data['segment'] = empty($get_segment_uri) ? 0 : $get_segment_uri;
		$student_list = $this->db->query("SELECT * FROM `commitee` WHERE `type` = 'm_member' ORDER BY `tec_id` ASC LIMIT ".$data['segment'].", ".$data['per_page']);
		...
		if (isset($_POST['filter_tec'])) {
			...
			$data['base_url'] = base_url().'commitee/index/';
			$data['total_rows'] = $this->db->query("SELECT * FROM `commitee` WHERE $search_name AND `type` = 'm_member' ORDER BY `tec_id`")->num_rows();
			$data['per_page'] = 10; 
			$data['uri_segment'] = 3;
			$data['num_links'] = 5;
			$get_segment_uri = $this->uri->segment(3);
			$data['segment'] = empty($get_segment_uri) ? 0 : $get_segment_uri;
			$student_list = $this->db->query("SELECT * FROM `commitee` WHERE $search_name AND `type` = 'm_member' ORDER BY `tec_id` DESC LIMIT ".$data['segment'].", ".$data['per_page']);
		}
		...
		if ($student_list->num_rows() > 0)
		{
		   $data['list_teacher'] = $student_list->result();
			 ...
			 $data['list_teacher'] = 'No Teacher Found';	
		}
		...
		$data['tec_path'] = "uploads/tec_image/";
		$data['web_page_title'] = "Managing Commitee Member List";
		...
		$this->load->view('front/member_comte', $data);
	...
	public function view($tec_id)
	{
		...
		$teacher_list = $this->db->query("SELECT * FROM `commitee` WHERE `tec_id` = '$tec_id'");
		...
		$this->load->view('front/header', $data);
		$this->load->view('front/sidebar-left', $data);
		$this->load->view('front/member_comte_detail', $data);
		$this->load->view('front/sidebar-right', $data);
		$this->load->view('front/footer', $data);
