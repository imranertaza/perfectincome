...
class agent extends CI_Controller {
	...
	public function listi()
	{
		...
		if ($this->login_status == true) {
			...
			$data['base_url'] = base_url().'admin_area/agent/listi/';
			$data['total_rows'] = $this->db->query("SELECT * FROM `users`,`user_roles` WHERE `users`.`ID`= `user_roles`.`userID` AND `user_roles`.`roleID` = '4' AND $name_src AND $phone_src ORDER BY `ID` DESC")->num_rows();
			$data['per_page'] = 10; 
			$data['uri_segment'] = 4;
			$data['num_links'] = 5;
			$get_segment_uri = $this->uri->segment(4);
			$data['segment'] = empty($get_segment_uri) ? 0 : $get_segment_uri;
			$all_student = $this->db->query("SELECT * FROM `users`,`user_roles` WHERE `users`.`ID`= `user_roles`.`userID` AND `user_roles`.`roleID` = '4' AND $name_src AND $phone_src ORDER BY `ID` DESC LIMIT ".$data['segment'].", ".$data['per_page']);
			$data['records'] = $all_student->result();
			$this->pagination->initialize($data);
			$data['pagination'] = $this->pagination->create_links();
			$this->load->view('admin/agent/listi', $data);
	...
	public function add()
	{
		...
		if ($this->login_status == true) {
			...
			$this->load->view('admin/agent/add', $data);
	...
	public function view($std_id)
	{
		...
		if ($this->login_status == true) {
			...
			$this->load->view('admin/agent/view', $data);
