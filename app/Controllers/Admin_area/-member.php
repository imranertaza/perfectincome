...
class member extends CI_Controller {
	...
	public function listi()
	{
		...
		if ($this->login_status == true) {
			...
			$data['base_url'] = base_url().'admin_area/member/listi/';
			$data['total_rows'] = $this->db->query("SELECT * FROM `users`,`user_roles` WHERE `users`.`ID`= `user_roles`.`userID` AND `user_roles`.`roleID` = '6' AND $name_src AND $phone_src ORDER BY `ID` DESC")->num_rows();
			$data['per_page'] = 10; 
			$data['uri_segment'] = 4;
			$data['num_links'] = 5;
			$get_segment_uri = $this->uri->segment(4);
			$data['segment'] = empty($get_segment_uri) ? 0 : $get_segment_uri;
			$all_student = $this->db->query("SELECT * FROM `users`,`user_roles` WHERE `users`.`ID`= `user_roles`.`userID` AND `user_roles`.`roleID` = '6' AND $name_src AND $phone_src ORDER BY `ID` DESC LIMIT ".$data['segment'].", ".$data['per_page']);
	...
	public function edit_student_subjects($std_id)
	{
		$data['std_id'] = $std_id;
		if ($this->login_status == true) {
			$this->load->view('admin/header');
			...
			$data['msg'] = '';
			/*if (isset($_POST['edit_student_subject'])) {
				$subject = empty($_POST['subject']) ? '0' : json_encode($_POST['subject']);
				$tech_subject = $_POST['tech_subject'];
				$update_subject = array(
				   'subjects' => $subject,
				   'tecsubject' => $tech_subject
				);
				$this->db->where('std_id', $std_id);
				$update = $this->db->update('students', $update_subject);
				if ($update) {
					$data['msg'] = '<p class="success">Subject Successfully Updated!</p>';
				}else{
					$data['msg'] = '<p class="error">Subject did not updated. Something wrong.</p>';
				}
			}*/
			
			
			$common_query = $this->db->query("SELECT * FROM `users` WHERE `ID` = '$std_id'");
			$data['rows'] = $common_query->row();
