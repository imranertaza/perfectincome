		...
		if (isset($_POST['confirm'])) {
			...
			if ($this->cart->total_items()>0) {  // If cart is empty
				if ($agent_existing_balance < $total_price) { // If don't have much balance
					...
					$spon_existing_commission = get_commission_by_id($data['spon_id']);
					...
					if ($com_taken_on_day < 40) {
						...
						if(!empty($l_t) && !empty($r_t)) {
							...
							if (($l_t_point >= $min_matching_point) && ($r_t_point >= $min_matching_point)) {
								...
								$each_side_com_point = $min_matching_point * $next_com_times;
								$total_point = $each_side_com_point*2;
								$matching_com = $matching_per_point * $total_point;
								...
								for($i=1; $i<=$next_com_times; $i++) {
									$data = array(
									   ...
									   'amount' => $total_point * $matching_per_point
									);
