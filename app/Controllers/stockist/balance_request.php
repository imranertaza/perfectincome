...
class balance_request extends CI_Controller {
	...
	public function index($user_id=0)
	{
		...
		if ($this->m_logged_in == true) {
			...
			$comment= $_POST['comment'];
				...
				$insert = $this->db->insert('balance_request' , $balance_req); 
				if ($insert){
						$data['msg'] .= '<p class="success">Requests Successfully Sent. </p>';
					}else {
						$data['msg'] .= '<p class="error">Requests failed...</p>';
					}
			...
			$this->load->view('front/client_area/stockist/balance_request', $data);
