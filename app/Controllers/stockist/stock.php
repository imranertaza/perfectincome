...
class stock extends CI_Controller {
	...
	public function index($user_id=0)
	{
		...
		$data['list_product'] = $this->db->query("SELECT * FROM `stockist_products_2` WHERE `stockist_id` = 15");
		...
		if ($this->m_logged_in == true) {
			...
			$data['query'] = $this->db->get_where('sales', array('agent_id' => $data['ID']));
			$this->load->view('front/client_area/stockist/stock', $data);
